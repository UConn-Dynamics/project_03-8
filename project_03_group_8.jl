...
# ╔═╡ 1b3c62a0-440a-11f1-aaeb-7327a03dd40e
begin
    using LinearAlgebra
...
# ╔═╡ 73c34ed8-879b-47c9-8dbb-95646664ff72
begin
    function Mmat()
        M = zeros(6, 6)
        M[1,1] = m1
        M[2,2] = m1
        M[3,3] = 1e-10      # θ1 fully constrained — near-zero to avoid singularity
        M[4,4] = m2
        M[5,5] = m2
        M[6,6] = I2
        return M
    ...
    function Fapp(q, qdot, t)
        x1 = q[1]
        F  = zeros(6)

        F[1] = -k * x1       # spring force on block
        F[2] = -m1 * grav    # gravity on block
        F[5] = -m2 * grav    # gravity on bar CM

        return F
    end

    "Mass matrix and forces defined"
end

# ╔═╡ 51b8d129-45bb-4896-8878-441884af0b14
begin
    function Cq_func(q, t)
        th2 = q[6]
        J   = zeros(4, 6)
        ...
        # C3: x2 - (L2/2)*sin(th2) - x1 = 0
        J[3, 1] = -1.0
        J[3, 4] =  1.0
        J[3, 6] = -(L2/2) * cos(th2)

        # C4: y2 - (L2/2)*cos(th2) = 0
        J[4, 5] =  1.0
        J[4, 6] =  (L2/2) * sin(th2)
...
# ╔═╡ 479e94b2-236d-4531-934a-440353c5477a
begin
    function eom!(du, u, p, t)
        n = 6    # number of coordinates
        m = 4    # number of constraints (4, not 5!)
        ...
        # γ: quadratic velocity terms (from differentiating constraints twice)
        # Derived from C3 and C4 — γ₁=γ₂=0 (linear constraints)
        gamma    = zeros(m)
        gamma[3] = -(L2/2) * sin(th2) * th2dot^2   # from C3
        gamma[4] = -(L2/2) * cos(th2) * th2dot^2   # from C4

        # Augmented system: (n+m) × (n+m)
        A_aug = [M        Cq';
                 Cq  zeros(m, m)]
        ...
        q_ddot = result[1:n]
        # lambda = result[n+1:end]  # constraint forces (Lagrange multipliers)
...
# ╔═╡ 889693ba-9a4d-43fb-828c-abc26a3e1727
begin
    th2_0 = 3* (pi/180)

    # Compute bar CM position from constraints at t=0
    x1_0  = 0.0
    x2_0  = x1_0 + (L2/2)*sin(th2_0)
    y2_0  =        (L2/2)*cos(th2_0)
...
# ╔═╡ 5ac274c6-e694-41cd-b71e-8107592f0f8c
begin
    using Printf
    ...
    anim = @animate for i in 1:5:length(sol.t)
        ...
        # Bar endpoints from CM position + angle
        x_bot = x2 - (L2/2)*sin(th2)
        y_bot = y2 - (L2/2)*cos(th2)
        x_top = x2 + (L2/2)*sin(th2)
        y_top = y2 + (L2/2)*cos(th2)

        # ── background / track ───────────────────────────────────────────
        plot([-1.5, 1.5], [0.0, 0.0],
             color=:black, lw=2, label="",
             xlims=(-1.2, 1.2), ylims=(-0.1, 0.7),
             aspect_ratio=:equal,
             title=@sprintf("t = %.2f s", sol.t[i]),
             xlabel="x [m]", ylabel="y [m]")

        # ── spring (zigzag between wall and block) ────────────────────────
        x_wall  = -0.8
        n_coils = 12
        xs = range(x_wall, x1 - 0.05, length=n_coils*2+1)
        ys = [j % 2 == 0 ? 0.02 : -0.02 for j in 0:n_coils*2]
        plot!(xs, ys, color=:gray, lw=1.5, label="")

        # ── block (rectangle centered at x1, y1=0) ───────────────────────
        bw, bh = 0.08, 0.06
        block_x = [x1-bw, x1+bw, x1+bw, x1-bw, x1-bw]
        block_y = [-bh/2, -bh/2,  bh/2,  bh/2, -bh/2]
        plot!(block_x, block_y,
              color=:steelblue, lw=2, fill=true, fillalpha=0.5, label="")

        # ── bar ───────────────────────────────────────────────────────────
        plot!([x_bot, x_top], [y_bot, y_top],
              color=:crimson, lw=4, label="")
        ...
        # ── bar CM marker ─────────────────────────────────────────────────
        scatter!([x2], [y2],
                 color=:crimson, markersize=4, label="")
    end

    gif(anim, "pendulum_block.gif", fps=30)
end

# ╔═╡ 0023e158-d363-4a99-bd0c-a93d0f57a64a
begin
    lambda_hist = zeros(length(sol.t), 4)

    for (i, (ti, ui)) in enumerate(zip(sol.t, sol.u))
        q    = ui[1:6]
        qdot = ui[7:12]
        th2  = q[6]
        th2d = qdot[6]
        ...
        gamma    = zeros(4)
        gamma[3] = -(L2/2)*sin(th2)*th2d^2
        gamma[4] = -(L2/2)*cos(th2)*th2d^2

        A_aug = [M       Cq';
                 Cq  zeros(4,4)]
        result = A_aug \ [F; gamma]

        lambda_hist[i, :] = result[7:10]
    end

    plot(sol.t, lambda_hist,
         label=["λ₁: track normal" "λ₂: no-rotation" "λ₃: hinge x" "λ₄: hinge y"],
         xlabel="Time [s]", ylabel="Force [N]",
         title="Constraint Forces (Lagrange Multipliers)",
         lw=2, size=(800, 400))
end

# ╔═╡ ba83c75c-8a65-4aa2-b360-8ecd005c21dc
begin
    t_plot   = sol.t
    x1_plot  = [sol.u[i][1]          for i in eachindex(sol.t)]
    th2_plot = [sol.u[i][6]*(180/pi) for i in eachindex(sol.t)]
    vx1_plot = [sol.u[i][7]          for i in eachindex(sol.t)]

    p1 = plot(t_plot, x1_plot,
              label="Block x₁",
              xlabel="Time [s]", ylabel="Position [m]",
              title="Block position vs time",
              lw=2, color=:steelblue)

    p2 = plot(t_plot, th2_plot,
              label="Bar angle θ₂",
              xlabel="Time [s]", ylabel="Angle [deg]",
              title="Bar angle vs time",
              lw=2, color=:crimson)

    p3 = plot(t_plot, vx1_plot,
              label="Block velocity ẋ₁",
              xlabel="Time [s]", ylabel="Velocity [m/s]",
              title="Block velocity vs time",
              lw=2, color=:darkorange)

    plot(p1, p2, p3; layout=(3,1), size=(800, 900))
end

# ╔═╡ c6909c93-db7e-4949-b464-a5fbe02724a8
begin
    function C_func(q, t)
        x1, y1, th1, x2, y2, th2 = q

        c1 = y1                             # block stays on track
        c2 = th1                            # block doesn't rotate
        c3 = x2 - (L2/2)*sin(th2) - x1    # hinge x: bar bottom = block x
        c4 = y2 - (L2/2)*cos(th2)          # hinge y: bar bottom at y=0
...
manifest_format = "2.0"
project_hash = "9ac9e85fda50d875c99ff98b02077819534fd1fa"
